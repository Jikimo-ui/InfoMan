...
<body>
    ...
    if (isset($_POST['RemoveSubmit'])) {
        ...
        if (empty($id)) {
            $errors[] = "Cashier ID is required.";
        } elseif (!preg_match('/^CS[0-9]{1,4}$/', $id)) {
            $errors[] = "Cashier ID must follow format CS0-CS9999.";
        }

        if (count($errors) > 0) {
            $_SESSION['errors_admin_cashier'] = $errors;
            header("Location: admin_cashier.php?action=remove");
            exit();
        }
        ...
        if ($mysqli->query($sql)) {
            if ($mysqli->affected_rows > 0) {
                $_SESSION['message_admin_view'] = "Cashier record removed successfully.";
                header("Location: admin_view.php");
                exit();
            } else {
                $_SESSION['errors_admin_cashier'] = ["No record found with that Cashier ID."];
                header("Location: admin_cashier.php?action=remove");
                exit();
            }
        } else {
            $_SESSION['errors_admin_cashier'] = ["Error removing record: " . $mysqli->error];
            header("Location: admin_cashier.php?action=remove");
            exit();
        }
    }
